...
st.set_page_config(page_title="Vehicle default", page_icon="https://www.sterlingbankasia.com/sites/default/files/2019-11/ultima%20inne-01.png", layout='wide', initial_sidebar_state='auto', menu_items=None)
...
def ModelTraining(data):
    ...
    NonEncodes = [income_type , education ,   occupation]
    Encodes = [income_type_label , education_label , occupation_label]
    ...
    return model , Encodes , NonEncodes
...
with mainContainer:
    ...
    if Submit:
        ...
        count1 = 0
        for i in NonEncodes[0]:
            if income_type == i:
                break
            else:
                count1+=1
        
        income_type = Encodes[0][count1]
    # ==========================================================================================
        count2 = 0
        for i in NonEncodes[1]:
            if education == i:
                break
            else:
                count2+=1
       
        education = Encodes[1][count2]

    # =============================================================================================

        count3 = 0
        for i in NonEncodes[2]:
            if occupation == i:
                break
            else:
                count3+=1
        
        occupation = Encodes[2][count3]

        inputs = [loan_amount , income , loan_annuity , Age , phone_change , ID_change , education , occupation , Gender
                ,income_type , child_count, phone_working , process_hour , city_rating , home_phone_tag , car_owned , Registration]
